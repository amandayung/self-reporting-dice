...
Amanda's note: Code has been modified for sending gyroscope and orientation
values via the photon cloud
*****************************************************************/
#include "SparkFunLSM9DS1/SparkFunLSM9DS1.h"
#include "math.h"
#include "SparkJson/SparkJson.h" //for sending data in JSON format
...
#define DECLINATION 11.45 // Declination (degrees) in Rochester, NY.

unsigned long startTime = 0;
unsigned long intervalCheck = 1000; //how often to send data to Photon cloud (1 sec is the minimum allowed)
...
void loop() {
    ...
    if (currentTime - startTime > intervalCheck) {
        ...
        imu.readAccel();
        imu.readMag();
        getAttitudeAndGyro(imu.ax, imu.ay, imu.az, -imu.my, -imu.mx, imu.mz, imu.gx, imu.gy, imu.gz);
  ...
  // axes are opposite to the accelerometer, so my and mx areor
...
// also get the gyroscope's values
void getAttitudeAndGyro(
float ax, float ay, float az, float mx, float my, float mz, float gx, float gy, float gz)
{
  ...
  //send the data
  sendData(pitch, roll, heading, gyroX, gyroY, gyroZ);
...
//send all the collected data to the cloud
void sendData(float pitch, float roll, float heading, float gyroX, float gyroY, float gyroZ) {
